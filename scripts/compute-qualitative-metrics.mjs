...
function computeMetrics(utterances, sentimentResults, audioDurationMs) {
  ...
  let userSentimentScores = userUtterances
...
async function main() {
  ...
  const toProcess = transcriptions
    .filter((t) => RECOMPUTE || !alreadyDone.has(t.call_id))
    .slice(0, LIMIT);
  ...
  for (const t of toProcess) {
    ...
    try {
      ...
      if (uErr) {
        ...
        const sentStr = metrics.user_sentiment_mean != null
          ? metrics.user_sentiment_mean.toFixed(3)
          : "n/a";
